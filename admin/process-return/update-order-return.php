...
if (isset($_POST["orderID"])) {
    ...
    if (empty($tableNo) or empty($orderContents) or empty($orderAmount)) {
        $result = '<div class="alert alert-warning">Lütfen boş alanları doldurun.</div>:::';
    } else {
        $updateOrder = $db->update(
            'UPDATE orderTable SET orderContents=?, tableNo=?, orderAmount=?, orderMessage=? WHERE orderID=?',
            [$orderContents, $tableNo, $orderAmount, $orderMessage, $orderID]
        );
        if ($updateOrder) {
            $result = '<div class="alert alert-success mt-2 mb-3 text-center" role="alert">
            <div style="font-size: 20px;">Sipariş başarı ile güncellendi</div>
            </div>:::';
        } else {
    ...
    foreach ($query as $items) {
        $result .= '<div id="orderID' . $items->orderID . '" style="display: none;">' . $items->orderID . '</div>
    <tr id="' . $items->orderID . '">
